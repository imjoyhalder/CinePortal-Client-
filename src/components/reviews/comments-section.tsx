...
import { FiMessageSquare, FiSend, FiCornerDownRight, FiLock, FiLoader } from "react-icons/fi";
...
export default function CommentsSection({ reviewId, pricing }: CommentsSectionProps) {
  ...
  // Resolve comment permission eagerly so there's no flash when the form opens
  useEffect(() => {
    if (!session) {
      setCanComment(false);
      return;
    }
    // Admins always can comment
    if (isAdmin) {
      setCanComment(true);
      return;
    }
    // Free-content: anyone logged in can comment
    if (!isPremium) {
      setCanComment(true);
      return;
    }
    // Premium content: need active paid subscription
    api
      .get<ApiResponse<Subscription | null>>("/payments/subscription")
      .then((r) => {
        const sub = r.data;
        setCanComment(sub?.status === "ACTIVE" && sub.plan !== "FREE");
      })
      .catch(() => setCanComment(false));
  }, [session, isAdmin, isPremium]);
        ...
        <div className="space-y-3">
          ...
          {!session && (
            <p className="text-xs text-muted-foreground">
              <Link href="/sign-in" className="underline hover:text-foreground">Sign in</Link> to comment
            </p>
          )}

          {/* Logged in — subscription still loading */}
          {session && canComment === undefined && (
          ...
          {/* Logged in — no access (free user on premium content) */}
          {session && canComment === false && (
            <div className="flex flex-col items-center gap-2.5 py-4 px-4 rounded-xl border border-amber-500/30 bg-amber-500/5 text-center mt-2">
              <FiLock className="w-4 h-4 text-amber-400" />
              <div>
                <p className="text-xs font-semibold">Subscription required</p>
                <p className="text-xs text-muted-foreground mt-0.5 leading-relaxed">
                  Only Monthly Pro and Annual Premium subscribers can comment on premium content.
                </p>
              </div>
